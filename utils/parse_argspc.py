...
def parse_args(description):
    ...
    parser.add_argument('--size', dest='size',
                        help='use data size', default=None, type=str)
    ...
    if args.size is not None:
        cfg_from_list(['DATASET.USE_SIZE', args.size])

    if len(cfg.MODEL_NAME) != 0 and len(cfg.DATASET_NAME) != 0 and len(cfg.TASK_NAME) != 0:
        outp_path = get_output_dir(cfg.MODEL_NAME, cfg.DATASET_NAME, cfg.TASK_NAME)
